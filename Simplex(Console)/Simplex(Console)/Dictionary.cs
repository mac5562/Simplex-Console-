...
namespace Simplex_Console_
{
    class Dictionary
    {
        ...
        /// <summary> /// basic variables /// </summary> public int[] basic;/// <summary> /// slack variables /// </summary> public int[] slack;
        public Dictionary(LPP lpp)
        {
            ...
            for (int i = 0; i < lpp.ObjFunc.VariablesNumber; i++)
                this.basic[i] = i + 1; this.slack = new int[lpp.Constraints.Length];
        ...
        private int LeavesBasis(int enterIdx, int preferToLeave = -1)
        {
            ...
            for (int i = 0; i < dc.Length; i++)
            {
                ...
                if (c[i, 1 + enterIdx] < 0) dc[i] = c[i, 0] / c[i, 1 + enterIdx];
                else dc[i] = double.NegativeInfinity;
            }
            double maxDC = dc[0];
        ...
        {// Recalculating coefficients for equation of entering variable 
            ...
            basic[enterIdx] = slack[leaveIdx];
            slack[leaveIdx] = valueToSwap;
